...
module TailwindMerge
  class ClassUtils
    ...
    def get_group_recursive(class_parts, class_part_object)
      ...
      class_part_object[:validators].find do |v|
    ...
    def get_prefixed_class_group_entries(class_group_entries, prefix)
    ...
    def get_class_part(class_part_object, path)
      path_parts = path.to_s.split(CLASS_PART_SEPARATOR)
    ...
    def get_group_id_for_arbitrary_property(class_name)
      if ARBITRARY_PROPERTY_REGEX.match?(class_name)
        ...
        property = arbitrary_property_class_name.split(":").first
